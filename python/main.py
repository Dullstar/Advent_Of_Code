...
def generate_days_using_report() -> list[DayToRun]:
    try:
        with open("report.txt", "r") as file:
            contents = file.read().replace("\n", " ")
    except FileNotFoundError:
        print("Didn't find an existing report. Running all days instead...")
        return generate_all_days()

    to_run = []
    # Matches a 4 digit number (the year) followed by a colon followed by ":" followed by a list of
    # 1 or 2 digit numbers (the list of days) and puts the year in match group 1 and the list in group 2.
    regex = re.compile(r"([0-9]{4}):\s*((?:[0-9]{1,2}(?:, )?)*)")
    for match in regex.finditer(contents):
        year = int(match[1])
        days = [int(day) for day in match[2].split(", ")]
        for day in days:
            to_run.append(DayToRun(year, day))
    return to_run
...
def interpret_args() -> list[DayToRun]:
    ...
    for i in range(1, len(sys.argv)):
        ...
        if arg == "all":
            ...
            to_run += generate_days_using_report()
        else:
            try:
                arg = int(arg)
...
def print_help() -> None:
    ...
    print("This file can also be generated by specifying \"--report\" which will skip days")
    print("that are already known to exist.")
...
def main() -> None:
    ...
    report = True if len(sys.argv) == 2 and (sys.argv[1] == "all" or sys.argv[1] == "--report") else False
    unimplemented = []
    failed = []
